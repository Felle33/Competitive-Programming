...
void solve(){
    ...
    while(right < n){
        res++;
        int vacciniRimasti = k;
        int firstArrived = timeArrive[right];

        while(right < n && firstArrived + w >= timeArrive[right] && vacciniRimasti > 0){
            right++;
            vacciniRimasti--;  
        } 

        int tempApertoPack = firstArrived + w;
        while(right < n && vacciniRimasti > 0 && timeArrive[right] <= tempApertoPack + d){
            right++;
            vacciniRimasti--;
        }
