...
void solve(){
    ...
    vector<ll> nums(n);
    vector<ll> cnt(n + m + 1);
    vector<ll> firstTime(n + m + 1);
    vector<ll> lastTime(n + m + 1);
    ...
    for(int i = 0; i < m; i++){
        ...
        lastTime[nums[pos - 1]] = i + 1;
        cnt[nums[pos - 1]] += lastTime[nums[pos - 1]] - firstTime[nums[pos - 1]];

        cin >> nums[pos - 1];
        firstTime[nums[pos - 1]] = i + 1;
    }


    for(int i = 0; i < n; i++){
        lastTime[nums[i]] = m + 1;
        cnt[nums[i]] += lastTime[nums[i]] - firstTime[nums[i]];
    }
