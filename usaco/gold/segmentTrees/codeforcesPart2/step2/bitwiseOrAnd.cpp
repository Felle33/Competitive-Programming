...
struct SegmentTree {
    ...
    ll NEUTRAL_ELEMENT = __LONG_LONG_MAX__;
    ...
    void or_bit(int l_query, int r_query, ll value, int x, int lx, int rx) {
        ...
        int mid = (lx + rx) / 2;
        or_bit(l_query, r_query, value, 2 * x + 1, lx, mid);
        or_bit(l_query, r_query, value, 2 * x + 2, mid, rx);
        tree[x] = (tree[2 * x + 1] & tree[2 * x + 2]) | op[x];
    }

    void or_bit(int l, int r, ll v) {
        or_bit(l, r, v, 0, 0, size);
    }
...
int main(){
    ...
    while(m--) {
        ...
        if(op == 1) {
            int l, r, v;
            cin >> l >> r >> v;
            segmentTree.or_bit(l, r, v);
        } else {
            int l, r;
            cin >> l >> r;
            cout << segmentTree.and_bit(l, r) << '\n';
        }
    }
}