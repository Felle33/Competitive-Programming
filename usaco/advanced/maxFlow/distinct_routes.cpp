...
struct Dinic {
    ...
    void dfs_print(int node, int parent, vvi& paths, int source, int sink) {
        if(node == sink) {
            paths.back().push_back(node);
            return;
        }

        for(int idEdge : adj[node]) {
            Edge& edge = edges[idEdge];
            if(edge.flo > 0 && edge.cap == edge.flo) {
                if(node == source) {
                    paths.push_back({node});
                    edge.flo = 0;
                    dfs_print(edge.to, node, paths, source, sink);
                } else {
                    paths.back().push_back(node);
                    edge.flo = 0;
                    dfs_print(edge.to, node, paths, source, sink);
    ...
    void print(int source, int sink) {
        vvi paths;
        dfs_print(0, -1, paths, source, sink);
